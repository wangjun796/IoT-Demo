...
export async function list(params: any) {
    return request(`/${globalConfig.apiPrefix}/open-api/_query`, {
        method: 'GET',
        params,
    });
}
...
export async function remove(id: string) {
    return request(`/${globalConfig.apiPrefix}/open-api/${id}`, {
        method: 'DELETE',
    });
}

export async function add(params: OpenApiItem) {
    return request(`/${globalConfig.apiPrefix}/open-api`, {
        method: 'POST',
        data: params,
    })
}
...
class Service extends BaseService<any>{
    ...
    public permission = {
        query: (params: any) => defer(() => from(
            request(`/${globalConfig.apiPrefix}/permission/_query/no-paging?paging=false`, {
                method: 'GET',
                params
            })).pipe(
        ...
        auth: (params: any) => defer(() => from(
            request(`/${globalConfig.apiPrefix}/autz-setting/_query/no-paging?paging=false`, {
                method: 'GET',
                params
            })).pipe(
                filter(resp => resp.status === 200),
        ...
        save: (data: any) => defer(() => from(
            request(`/${globalConfig.apiPrefix}/autz-setting/detail/_save`, {
                method: 'POST',
                data
            })).pipe(
                filter(resp => resp.status === 200),
