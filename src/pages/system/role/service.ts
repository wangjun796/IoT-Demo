...
class RoleService<T> extends Service<T> {
  public bindUser = (params: any) => from(request(`/${globalConfig.apiPrefix}/dimension-user/_query/no-paging`, {
    method: 'GET',
    params
  })).pipe(map((response: ApiResponse<T>) => response, catchError(error => of(error))));

  public unBindUser = (id: string) => from(request(`/${globalConfig.apiPrefix}/dimension-user/${id}`, {
    method: 'DELETE',
  })).pipe(map((response: ApiResponse<T>) => response, catchError(error => of(error))));

  public bind = (params: any) => from(request(`/${globalConfig.apiPrefix}/dimension-user`, {
    method: "POST",
    data: params
  })).pipe(map((response: ApiResponse<T>) => response, catchError(error => of(error))));
...
export async function list(params: any) {
  return request(`/${globalConfig.apiPrefix}/dimension/_query`, {
    method: 'GET',
    params,
  });
}
