...
class Service extends BaseService<UserDetail>{
    ...
    public setMainTenant = (tenant: string) => defer(() => from(
        request(`/${globalConfig.apiPrefix}/tenant/${tenant}/_main`, {
            method: 'PUT'
        })).pipe(
            map(resp => resp.result)
        ));
    ...
    public notificationProvider = () => defer(() => from(
        request(`/${globalConfig.apiPrefix}/notifications/providers`)
    )).pipe(
        map(resp => resp.result)
    );

    public deviceAlarm = (params: any) => defer(() => from(
        request(`/${globalConfig.apiPrefix}/device/alarm/_query`, {
            method: 'GET',
            params
        })
    )).pipe(
    ...
    public saveSubscribe = (data: any) => defer(() => from(
        request(`/${globalConfig.apiPrefix}/notifications/subscribe`, {
            method: 'PATCH',
            data
        })
    )).pipe(
    ...
    public notification = {
        close: (id: string) => defer(() => from(
            request(`/${globalConfig.apiPrefix}/notifications/subscription/${id}/_disabled`, {
                method: 'PUT',
            })
        )),

        open: (id: string) => defer(() => from(
            request(`/${globalConfig.apiPrefix}/notifications/subscription/${id}/_enabled`, {
                method: 'PUT',
            })
        )),

        remove: (id: string) => defer(() => from(
            request(`/${globalConfig.apiPrefix}/notifications/subscription/${id}`, {
                method: 'DELETE',
            })
        )),
    }
