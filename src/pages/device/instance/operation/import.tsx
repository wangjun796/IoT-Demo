...
const Import: React.FC<Props> = props => {
  ...
  const submitData = (fileUrl: string) => {
    if (fileUrl) {
      ...
      const source = new EventSourcePolyfill(
        wrapAPI(`/${globalConfig.apiPrefix}/device/instance/${product}/import?fileUrl=${fileUrl}&:X_Access_Token=${getAccessToken()}`)
      );
      setSource(source);
  ...
  const downloadTemplate = (type: string) => {
    ...
    formElement.action = `/${globalConfig.apiPrefix}/device/instance/${product}/template.${type}`;
