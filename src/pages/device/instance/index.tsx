...
const DeviceInstancePage: React.FC<Props> = props => {
    ...
    apis.deviceInstance.count(encodeQueryParam({ terms: { state: 'notActive', productId, ...location?.query?.terms, } }))
      .then(res => {
        if (res.status === 200) {
          ...
          // alert(JSON.stringify({...map}));//useState的setter是异步的
          setDeviceCount({ ...map });
        }
      }).catch();
  ...
  useEffect(() => {
    ...
    if (query.hasOwnProperty('productId')) {//url带query
        ...
        <Card bordered={false} style={{ height: 95 }}>
          <Spin spinning={deviceCount.loading}>
            <Row>
              ...
              <Col sm={4} xs={24}>
                <Info title={<Badge status={statusMap.get('默认')} text="全部设备" />}
