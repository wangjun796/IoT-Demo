...
const DeviceInstancePage: React.FC<Props> = props => {
  ...
  const startImport = () => {
    ...
    const activeAPI = `/${globalConfig.apiPrefix}/device-instance/deploy?${getSearchParam()}:X_Access_Token=${getAccessToken()} `;
    setAPI(activeAPI);
    setAction('active');
  };
  ...
  const startSync = () => {
    setProcessVisible(true);
    const syncAPI = `/${globalConfig.apiPrefix}/device-instance/state/_sync/?${getSearchParam()}:X_Access_Token=${getAccessToken()}`;
    setAPI(syncAPI);
    setAction('sync');
  };
  ...
  const exportDevice = () => {
    ...
    formElement.action = `/${globalConfig.apiPrefix}/device-instance/export?:X_Access_Token=${getAccessToken()}`;
  ...
  const uploadProps: UploadProps = {
    accept: '.xlsx',
    action: `/${globalConfig.apiPrefix}/file/static`,
    headers: {
    ...
    onChange(info) {
      if (info.file.status === 'done') {
        ...
        const url = `/${globalConfig.apiPrefix}/device-instance/import?fileUrl=${fileUrl}&:X_Access_Token=${getAccessToken()}`;
        setAPI(url);
        setAction('import');
